...
生产级黄金市场实时分析页面，包含实时价格、图表、止损点、结构化买入推荐、新闻情绪和盈亏计算。页面默认按国内常用口径显示人民币/克，并保留原始 XAU/USD 报价。行情默认优先读取 Yahoo Finance `GC=F`，也支持在前端切换 Yahoo 现货、GoldAPI 和 demo 参考源；访问失败时回退到 2026-06-26 更新的参考价。
