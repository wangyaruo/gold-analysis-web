...
<template>
  <main class="app-shell">
    <header class="topbar">
      <div>
        <p class="eyebrow">CNY/g Gold Real-Time Desk</p>
        <h1>黄金市场实时价格分析</h1>
      </div>
    ...
    <section class="price-band">
      <div class="price-main">
        ...
        <div class="price-row">
          ...
          <span class="currency">{{ displayUnit }}</span>
        </div>
        <div class="meta-row">
      ...
      <div class="recommendation" :class="`recommendation-${recommendation.action}`">
        ...
        <strong data-testid="recommendation-action">{{
            recommendation.action === 'buy' ? '建议买入' : '暂不买入'
          }}</strong>
    ...
    <section class="dashboard-grid">
      ...
      <article class="panel metrics-panel">
        ...
        <dl class="metric-list">
          ...
          <div>
            <dt>单位</dt>
            <dd>{{ snapshot?.indicators?.stop_loss?.display_unit || displayUnit }}</dd>
          </div>
        </dl>
      </article>
